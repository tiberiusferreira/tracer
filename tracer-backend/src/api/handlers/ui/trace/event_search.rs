...
pub async fn search(
    ...
    let start_timestamp = u64_nanos_to_db_i64(trace_chunk.chunk_id.start_timestamp)?;
    let end_timestamp = u64_nanos_to_db_i64(trace_chunk.chunk_id.end_timestamp)?;
...
from (select *
    ...
    // let quality = 4;
    // let json = serde_json::to_string(&events).expect("to be able to serialize response");
    // let mut input =
