...
pub(crate) async fn insert_events(
    ...
    // }
    // let instance_ids: Vec<Uuid> = new_events
    //     .iter()
    //     .map(|_s| instance_id.instance_id)
    //     .collect();
    // let trace_ids: Vec<i32> = new_events.iter().map(|_s| trace_id).collect();
    //
    // let timestamps: Vec<NaiveDateTime> = new_events
    //     .iter()
    //     .map(|s| time_from_nanos(s.timestamp))
    //     .collect();
    // let span_ids: Vec<i32> = new_events.iter().map(|s| s.span_id as i32).collect();
    ...
    //     .collect();
    // let severities: Vec<Severity> = new_events
    //     .iter()
    //     .map(|s| Severity::from(s.severity))
    //     .collect();
    // let db_event_ids: Vec<i32> = match sqlx::query_scalar!(
    //         "insert into event (instance_id, trace_id, span_id, timestamp, message, severity, module, filename, line)
    //         select * from unnest($1::UUID[], $2::INT[], $3::INT[], $4::timestamp[], $5::TEXT[], $6::severity_level[], $7::TEXT[], $8::TEXT[], $9::INT[]) returning id;",
    //         &instance_ids,
    //         &trace_ids,
    //         &span_ids,
    //         &timestamps,
    ...
    //         select * from unnest($1::UUID[], $2::BIGINT[], $3::BIGINT[], $4::INT[], $5::TEXT[], $6::TEXT[]);",
    //         &kv_instance_id,
    //         &kv_trace_id,
    //         &kv_span_id,
    //         &kv_event_id,
