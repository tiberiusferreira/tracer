...
#[derive(Debug, Clone)]
pub struct ServerExporterConfig {
...
impl ServerExporterConfig {
    pub fn new(service_id: ServiceId, collector_url: String) -> ServerExporterConfig {
        ServerExporterConfig {
            collector_url,
...
pub async fn setup_server_exporter_task_or_panic(config: ServerExporterConfig) -> ExporterHandle {
    info!("Starting up using: {config:#?}");
...
async fn registration_loop(
    ...
    loop {
        info!("Sending tracer registration request");
            ...
            .await
        {
            ...
            Err(err) => {
                ...
                info!(
                    "Registration failure: {err_str} - sleeping {sleep_seconds}s",
                );
...
async fn setup_server_exporter_or_panic_impl(config: ServerExporterConfig) -> TracerTasks {
    ...
    let registration_response = registration_loop(
        ...
        .await;
    info!("registered with collector");
    ...
    loop {
        ...
        let flush_request = tokio::select! {
            ...
            received_val = flush_request_receiver.recv() => {
                match received_val{
                    Some(flush_request) => {
                        info!(context, "Got flush request");
                        Some(flush_request)
                    }
                    None => {
                        info!(context, "Flush request channel is closed, sleeping");
        ...
        info!(context, "Checking for new events");
        ...
        let execution_recording = get_global_recorder().get_all_pruning();
        let api_execution_recordings: Vec<api_structs::instance::update::ExecutionRecordingSnapshot> = execution_recording.into_iter().map(|e| api_structs::instance::update::ExecutionRecordingSnapshot {
            id: e.id,
            started_at: e.started_at,
            last_seen_at: e.last_seen_at,
            ended: e.ended,
            execution_io_fragment: api_structs::instance::update::ExecutionIOFragment {
                input: e.execution_io_fragment.input,
                io_providers_events: e.execution_io_fragment.io_providers_events.into_iter().map(|(recorder_name, events)| {
                    let events = events.into_iter().map(|e| api_structs::instance::update::IoEvent {
                        id: e.id,
        ...
        let export_data = api_structs::instance::update::InstanceSnapshot {
            instance_id,
            execution_recordings: api_execution_recordings,
            cpu_profile_base64,
        };
        trace!(context, "Export data: {:#?}", export_data);
        let export_data_json =
            serde_json::to_string(&export_data).expect("export data to be serializable");
        loop {
            info!(context, "attempting export");
            tokio::time::sleep(period_time_secs).await;
        ...
        flush_request.map(|f| match f.respond_to.send(Ok(())) {
            Ok(_) => {
                info!("Responded to flush request");
            }
            Err(e) => {
                info!(
