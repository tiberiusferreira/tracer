...
pub async fn insert_or_update_trace(
    ...
    match existing_trace_and_open_spans {
        None => {
            ...
            let mut spans_to_upsert = validate_spans_for_insertion(trace_fragment)?;
        ...
        Some(existing) => {
            ...
            info!(trace.id=existing.id.to_string(), trace.trace_count_id = existing.trace_count_id, "existing trace found");
            ...
            let mut spans_to_upsert = validate_spans_for_insert_or_update(&existing, trace_fragment)?;
            upsert_spans(&mut *tx, instance_update_id,  existing.id, &mut spans_to_upsert).await?;
        }
    };
...
async fn upsert_spans(tx: &mut edgedb_tokio::Transaction, instance_update_id: uuid::Uuid, trace_id: uuid::Uuid, spans_to_upsert: &mut SpansToUpsert) -> Result<(), TraceUpdateError> {
    edgedb_query!(span_insert, "with
...
for single_span in json_array_unpack(span_data) union (
  ...
  insert Span  {
    ...
    name := name,
    parent := (
      if exists single_span['parent_id'] then (
    ...
    spans_to_upsert.spans_to_insert.sort_unstable_by_key(|e| e.id);
    let spans_to_insert_json_str = serde_json::to_string(&spans_to_upsert.spans_to_insert).unwrap();
    info!(spans_to_insert_json_str ,"spans to insert");
    let res = span_insert::transaction(&mut *tx, &span_insert::Input{
    ...
    for item in json_array_unpack(raw_data) union (
      with attributes := json_object_unpack(item['attributes_to_add']),
      new_attributes := (
        for attr in attributes union (
          insert Attribute{
            name := (
              insert AttributeName {
               name := attr.0
      ...
      set {
        duration_nanos := <int64>item['duration_nanos'],
        attributes := new_attributes
      }
    );
...
fn validate_spans_for_insertion(
