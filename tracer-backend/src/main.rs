...
use tracer::server_exporter::ServerExporterConfig;
...
async fn main() {
    ...
    let tracer_config = ServerExporterConfig::new(
        ServiceId {
    ...
    current_thread_runner
        .run_until(async {
            ...
            let tracer_flush_request =
                tracer::server_exporter::setup_server_exporter_task_or_panic(tracer_config).await;
...
async fn start_api_and_background_tasks(
    ...
    let execution_io_provider = gel_io_provider::DatabaseIoRecorder::Live(edgedb_client);
        ...
        loop {
            let task = async {
                ...
                if let Err(e) = outcome {
                    let error_str = tracer::application_api::record_error(e);
