...
impl PgIoRecorder {
    pub async fn query_optional<
        ...
        T: Serialize + DeserializeOwned + Clone + Send + Unpin + for<'r> FromRow<'r, <Postgres as sqlx::Database>::Row>,
    >(
        &self,
        query: &str,
        ...
        let raw_io_response: Result<Option<T>, Error> = raw_query_optional(client, query, parameters).await;
        record_io_response_as_query_result(recorded_io_req, raw_io_response.clone());
        raw_io_response
    }
}

async fn raw_query_optional<T: Serialize + DeserializeOwned + Clone + Send + Unpin + for<'r> FromRow<'r, <Postgres as sqlx::Database>::Row>>(con: &PgPool, query: &str, parameters: IndexMap<String, Parameter>) -> Result<Option<T>, Error> {
