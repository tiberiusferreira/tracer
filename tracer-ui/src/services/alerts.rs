    ...
    for (trace_name, trace_alert_config) in service_alert_config_trace_overwrite {
        let row = view! {
            <tr class="row-container">
                <th class="trace-table__cell">{trace_name}</th>
                <th class="trace-table__cell">{trace_alert_config.max_trace_duration_ms}</th>
    ...
    view! {
        <div>
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 20px; justify-content: center">
                <p style="margin: 0px">
                    {"Min Instance Count: "}<b>{service_wide.min_instance_count}</b>
                </p>
                <p style="margin: 0px">
                    {"Max Active Traces: "}<b>{service_wide.max_active_traces_count}</b>
                </p>
            </div>
            <p style="text-align: center">{"Per Trace Global Alerts:"}</p>
            <div style="display: flex; flex-wrap: wrap; gap: 20px; justify-content: center">
                <p style="margin: 0px">
                    {"Percentage Check Time Window sec: "}
                    <b>{trace_wide.percentage_check_time_window_secs}</b>
                </p>
                <p style="margin: 0px">
                    {"Percentage Check Min Samples: "}
                    <b>{trace_wide.percentage_check_min_number_samples}</b>
                </p>
                <p style="margin: 0px">
                    {"Max Duration: "}<b>{trace_wide.max_trace_duration_ms}</b>"ms"
                </p>
                <p style="margin: 0px">
                    {"Max Warning: "}<b>{trace_wide.max_traces_with_warning_percentage}</b>"%"
                </p>
            </div>
